module Api
  module V1
    class DashboardController < ApiController
      def data
        now = DateTime.now()
        @expenses = Expense.where(account_id: current_user.accounts.first.id).last(5)
        @incomes = Income.where(account_id: current_user.accounts.first.id).last(5)
        @work_hours = WorkHour.where(account_id: current_user.accounts.first.id).last(5)
