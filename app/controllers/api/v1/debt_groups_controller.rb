...
module Api
  module V1
    class DebtGroupsController < ApiController
      ...
      def data
        groups = DebtGroup.where(account_id: current_user.accounts.first.id).order("name ASC").pluck(:name)
      ...
      def all
        groups = DebtGroup.where(account_id: current_user.accounts.first.id).order("name ASC")
